  ...
  try {
    ...
    const options = {
      ...
      handler: async function (response) {
        ...
        try {
          const verifyRes = await fetch("https://tripsera-web-backend.vercel.app/api/verify-payment", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
...
{isSelected && (
  ...
  <img 
    src={el.src.startsWith('http') 
      ? `https://tripsera-web-backend.vercel.app/api/proxy?url=${encodeURIComponent(el.src)}` 
      : el.src} 
